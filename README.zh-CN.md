...
PocketSSH 是一个运行在 Android 设备本地的轻量 SSH 服务端项目，面向局域网设备维护、调试、文件传输和自动化控制场景。应用提供前台 SSH 服务、深蓝暗黑科技风配置页面、开机自启动支持，并兼容常见桌面 SSH/SFTP/SCP 客户端。

PocketSSH 采用宽松的 MIT 许可证完全开源，允许个人使用、商业使用、修改、再分发以及集成到其他产品，但须按许可证要求保留版权和许可声明。
...
```text
PocketSSH-v{versionName}.{versionCode 四位补零}.apk
```
...
- PocketSSH 不是系统 root shell，默认以应用自身权限执行命令。
...
PocketSSH 是采用 [MIT License](LICENSE.md) 发布的开源软件。个人和组织均可使用、修改、分发、再许可及商业使用，无需另行取得作者授权，但必须保留版权和许可声明。
